...
| ----------------------------- | ------------------------------------------------------------ | ------------------------------------- | --------------------------------------------------- |
| Encrypt a file                | `pylock file.txt --encrypt`                                  | `pylock file.txt -e`                  | Encrypts the file and saves it as `file.txt.pylock` |
| Decrypt a file                | `pylock file.txt.pylock --decrypt`                           | `pylock file.txt.pylock -d`           | Decrypts the `.pylock` file back to original        |
| Ecrypt + delete Original file | `pylock file.txt --encrypt --delete-original`                | `pylock file.txt -e -D`               | Encrypt and delete source file                      |
...
| Decrypt with password         | `pylock file.txt.pylock --decrypt -p password123`            | `pylock file.txt.pylock -d -p xyz`    | Provide password directly (not recommended)         |
| Show help                     | `pylock --help`                                              | `pylock -h`                           | Display help message                                |
